  ...
  methods: {
    ...
    update: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        const issueId = existing.key ?? existing.id;
        const credentials = { site: g.site, email: g.email, token: g.token };
        let result: ResourceData;
        if (g.transition !== undefined) {
          // Transitions must go through POST /issue/{id}/transitions — a plain
          // PUT /issue/{id} with a `transition` key is silently rejected.
          const transitionBody: Record<string, unknown> = {};
          if (g.fields !== undefined) transitionBody.fields = g.fields;
          if (g.update !== undefined) transitionBody.update = g.update;
          if (g.properties !== undefined) {
            transitionBody.properties = g.properties;
          }
          result = await transitionIssue(
            issueId,
            g.transition as Record<string, unknown>,
            transitionBody,
            credentials,
          ) as ResourceData;
        } else {
