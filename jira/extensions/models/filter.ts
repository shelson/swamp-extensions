...
// Auto-generated extension model for @shelson/jira/filter
// Do not edit manually. Re-generate with: deno task generate:jira

// deno-lint-ignore-file no-explicit-any

/**
 * Swamp extension model for a Jira filter.
 *
 * Wraps the `/rest/api/3/filter` API as a swamp model so create, get, update,
...
const GlobalArgsSchema = z.object({
  ...
  })).describe("The groups and projects that can edit the filter.").optional(),
  favourite: z.boolean().describe(
  ...
  })).describe("The groups and projects that the filter is shared with.")
    .optional(),
...
/** Swamp extension model for Jira filter. Registered at `/jira/filter`. */
export const model = {
  type: "/jira/filter",
  ...
  resources: {
    state: {
      description: "Filter resource state",
  ...
  methods: {
    create: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g.favourite !== undefined) body.favourite = g.favourite;
        if (g.jql !== undefined) body.jql = g.jql;
        if (g.name !== undefined) body.name = g.name;
        if (g.sharePermissions !== undefined) {
          body.sharePermissions = g.sharePermissions;
        }
        const result = await create("/rest/api/3/filter", body, {
          site: g.site,
          email: g.email,
          token: g.token,
        }) as ResourceData;
    ...
    get: {
      ...
      arguments: z.object({
        id: z.union([z.string(), z.number()]).describe("The ID of the filter"),
      }),
      execute: async (args: { id: string | number }, context: any) => {
        const g = context.globalArgs;
        const result = await read("/rest/api/3/filter", args.id, {
          site: g.site,
          email: g.email,
          token: g.token,
        }) as ResourceData;
    ...
    update: {
      description: "Update filter attributes",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const instanceName = (g.name?.toString() ?? "current").replace(
    ...
    delete: {
      ...
      execute: async (args: { id: string | number }, context: any) => {
        const g = context.globalArgs;
        const { existed } = await remove("/rest/api/3/filter", args.id, {
          site: g.site,
          email: g.email,
          token: g.token,
        });
    ...
    sync: {
      description: "Sync filter state from Jira",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const instanceName = (g.name?.toString() ?? "current").replace(
