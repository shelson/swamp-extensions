...
// Auto-generated extension model for @shelson/jira/resolution
  ...
  methods: {
    ...
    delete: {
      ...
      arguments: z.object({
        ...
        replaceWith: z.union([z.string(), z.number()]).optional().describe(
          "ID of the resolution to move issues to (required when the resolution is in use)",
        ),
      }),
      execute: async (
        args: { id: string | number; replaceWith?: string | number },
        context: any,
      ) => {
        const g = context.globalArgs;
        context.logger.info("Running {method} on {type}", {
        ...
        const query = args.replaceWith !== undefined
          ? { replaceWith: String(args.replaceWith) }
          : undefined;
        const { existed } = await removeWithQuery(
          "/rest/api/3/resolution",
          args.id,
          query,
          {
